...
char* indexes(const char* path)
{
    ...
    char* php = "index.php";
    char* html_index = malloc(strlen(html) + strlen(path) + 1);
    strcpy(html_index, path);
    strcat(html_index, html);
    
    char* php_index = malloc(strlen(php) + strlen(path) + 1);
    strcpy(php_index, path);
    strcat(php_index, php);
    
    if(access(html_index, F_OK) == 1)
    {
        return html_index;
    }
    else if(access(php_index, F_OK) == 1)
    {
        return php_index;
    }
    return NULL;
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *content = NULL;
    ...
    for(int i = fread(buffer, 1, 512, file); i != 0; i = fread(buffer, 1, 512, file))
    {
...
const char* lookup(const char* path)
{
    ...
    // check's extensions and return MIME type for supported 
    if (extension != NULL)
    {
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // check request-line to start with '/'
    if (line[4] != '/')
    {
        error(501);
        return false;       
    ...
    // check for errors in http_version
    ...
    // try to find query in request
    needle = strchr(request, '?');
